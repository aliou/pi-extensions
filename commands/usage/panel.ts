...
import type { ProviderSnapshot } from "@harness/provider-usage";
import { buildProviderTab } from "./provider-tab";
import { ensureWidth, truncateSafe } from "./utils";
...
export class UsagePanel implements Component {
  ...
  constructor(
    ...
    this.tabs = sortSnapshots(snapshots, activeProvider).map((s) => ({
      label: s.displayName,
      snapshot: s,
    }));
  }
